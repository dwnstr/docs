...
This second image shows the same mesh but scaled down. Notice how the object moved toward the origin. This is because the origin is the reference point for transforms.
If the origin was in the center of the mesh rather than behind it, the mesh would have scaled in place rather than appearing to move.
...
Parenting is the relationship between two objects where one object is the parent, and one is the child. The child object will inherit the parent's transforms.
An obect can have multiple children, but only one parent.

Parenting is often represented in 3D software as nested objects.
